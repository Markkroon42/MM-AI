...
@section('title', 'Edit Scheduled Task: ' . $task->name)

@section('content')
<div class="mb-4">
    <h1 class="h2">Edit Scheduled Task: {{ $task->name }}</h1>
</div>
        ...
        <form method="POST" action="{{ route('admin.scheduled-tasks.update', $task) }}">
            ...
            <div class="mb-3">
                ...
                <select name="task_type" id="task_type" class="form-select @error('task_type') is-invalid @enderror" required>
                    ...
                    <option value="run_agent" {{ old('task_type', $task->task_type) == 'run_agent' ? 'selected' : '' }}>Run Agent</option>
                    <option value="generate_report" {{ old('task_type', $task->task_type) == 'generate_report' ? 'selected' : '' }}>Generate Report</option>
                    <option value="create_kpi_snapshot" {{ old('task_type', $task->task_type) == 'create_kpi_snapshot' ? 'selected' : '' }}>Create KPI Snapshot</option>
                    <option value="sync_meta" {{ old('task_type', $task->task_type) == 'sync_meta' ? 'selected' : '' }}>Sync Meta Data</option>
                    <option value="cleanup_old_data" {{ old('task_type', $task->task_type) == 'cleanup_old_data' ? 'selected' : '' }}>Cleanup Old Data</option>
                </select>
                @error('task_type')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="cron_expression" id="cron_expression"
                           ...
                           value="{{ old('cron_expression', $task->cron_expression) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                        ...
                        <option value="paused" {{ old('status', $task->status) == 'paused' ? 'selected' : '' }}>Paused</option>
                        <option value="disabled" {{ old('status', $task->status) == 'disabled' ? 'selected' : '' }}>Disabled</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-3">
                ...
                <textarea name="run_context_json" id="run_context_json" rows="6"
                          class="form-control font-monospace @error('run_context_json') is-invalid @enderror">{{ old('run_context_json', json_encode($task->run_context_json ?? [], JSON_PRETTY_PRINT)) }}</textarea>
            ...
            <div class="mb-4">
                <div class="form-check form-switch">
                    <input type="checkbox" name="alert_on_failure" id="alert_on_failure" value="1"
                           class="form-check-input" {{ old('alert_on_failure', $task->alert_on_failure) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-between">
                <a href="{{ route('admin.scheduled-tasks.show', $task) }}" class="btn btn-outline-secondary">
                    <i class="bi bi-x-circle me-1"></i> Cancel
                </a>
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-circle me-1"></i> Update Scheduled Task
                </button>
            </div>
        </form>
    </div>
