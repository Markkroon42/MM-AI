...
@section('title', 'Create Guardrail Rule')

@section('content')
<div class="mb-4">
    <h1 class="h2">Create Guardrail Rule</h1>
</div>
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.guardrail-rules.store') }}">
            @csrf
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="priority" id="priority" class="form-control @error('priority') is-invalid @enderror"
                           value="{{ old('priority', 100) }}" min="1" required>
                    @error('priority')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select name="applies_to_action_type" id="applies_to_action_type"
                        ...
                        <option value="campaign_create" {{ old('applies_to_action_type') == 'campaign_create' ? 'selected' : '' }}>Campaign Create</option>
                        <option value="campaign_update" {{ old('applies_to_action_type') == 'campaign_update' ? 'selected' : '' }}>Campaign Update</option>
                        <option value="campaign_delete" {{ old('applies_to_action_type') == 'campaign_delete' ? 'selected' : '' }}>Campaign Delete</option>
                        <option value="budget_update" {{ old('applies_to_action_type') == 'budget_update' ? 'selected' : '' }}>Budget Update</option>
                        <option value="adset_create" {{ old('applies_to_action_type') == 'adset_create' ? 'selected' : '' }}>Ad Set Create</option>
                        <option value="adset_update" {{ old('applies_to_action_type') == 'adset_update' ? 'selected' : '' }}>Ad Set Update</option>
                        <option value="ad_create" {{ old('applies_to_action_type') == 'ad_create' ? 'selected' : '' }}>Ad Create</option>
                        <option value="ad_update" {{ old('applies_to_action_type') == 'ad_update' ? 'selected' : '' }}>Ad Update</option>
                        <option value="publish_job" {{ old('applies_to_action_type') == 'publish_job' ? 'selected' : '' }}>Publish Job</option>
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select name="effect" id="effect" class="form-select @error('effect') is-invalid @enderror" required>
                        ...
                        <option value="warn" {{ old('effect') == 'warn' ? 'selected' : '' }}>Warn</option>
                        <option value="require_approval" {{ old('effect') == 'require_approval' ? 'selected' : '' }}>Require Approval</option>
                        <option value="block" {{ old('effect') == 'block' ? 'selected' : '' }}>Block</option>
                    </select>
                    @error('effect')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="mb-3">
                ...
                <textarea name="condition_expression" id="condition_expression" rows="6"
                          ...
                          required>{{ old('condition_expression') }}</textarea>
            ...
            <div class="d-flex justify-content-between">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-circle me-1"></i> Create Guardrail Rule
                </button>
            </div>
        </form>
    </div>
